...
%
% Answer: removes items from your workspace 
%
...
%
% Answer: while loops- repeatedly execute the program as long as the
% expression remains true 
% for loops-  a repetition control structure that allows you to efficiently 
...
%
% Answer: not a number. when numeric results are undefinable ex. 0/0= NaN
%
...
% - find the maximum.
V = [1 3 7 9 6] %created vector 
T = (V-3) %subtract 3 from each 
POS = V(V>0) %positive 
abs(V) %absolute value of each 
M = max(V) %maximum value 
